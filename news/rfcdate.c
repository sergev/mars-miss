...
extern int strcmp (), sprintf ();

/*
 * Занесение в буфер имени временной зоны.
 */
static void settz (register char *b, register int tz)
{
	if (tz == 0) {
		strcpy (b+24, "GMT");
		return;
 ...
 * Чтение целого числа.
 */
static int getint (register char **p)
{
	register n;
 ...
 * иначе 0.
 */
static int getkeyword (register char **p, struct keytab *tab, int tabsz)
{
	register n;
	char key [20];
 ...
 */
static int weekday (int day, int mon, int year)
{
 ...
 */
char *genrfcdate (long tim, int tz)
{
	return (ctim2rfc (ctime (&tim), tz));
}

/*
 * Выборка полей даты из текстовой строки.
 */
int parserfcdate (char *ctim, int *pyear, int *pmon, int *pday,
...
char *rfcdate (char *ctim)
{
	...
	if (parserfcdate (ctim, &year, &mon, &day, &h, &m, &s, &tz, &wday)) {
